...
import numba
import pickle
from carsharing_repos import CarSharingR
##############################################################################
...
class Qlearning():
    ...
    def __init__(self, ENV,
                       ...
                       eps_greedy_par=0.5,
                       NUM_STEPS=30001,
                       interactive_plot_states=[(5,3),(6,7),(7,9)],
                       num_to_save_Q=1000,
                       polynomial_lr = 0.5
                       ):
        self.env = ENV
    ...
    def epsilon_greedy_policy(self, q_values, force_epsilon=None):
        ...
        if self.prng.rand() < eps:
             ...
             action = self.env.feas_actions[self.env.observation,action_idx]
        else:
            ...
            if verbose:    
                print('Step:',step, 'reward:',"{:.2f}".format(reward), 
                      'epsilon:', "{:.2f}".format(self.epsilon),'action_id:', action_idx, 'state:', state, )          
                       ...
                       #'|Qstar-Q|=',"{:.2f}".format(np.linalg.norm(Qstar - self.Q)),
                      #'|Vstar-V|=',"{:.2f}".format(np.linalg.norm(np.max(Qstar, axis=1) - np.max(self.Q, axis=1))))
            state = newState
...
class Bias_corrected_QL(Qlearning):
    ...
    def train(self, verbose = False):
         ...
         for step in range(self.num_steps):
            ...
            #else: # self.Rmean[state, action_idx] instead of reward
             #   delta = reward - Q[state, action_idx]
...
class Replay_Memory():
    ...
    def __init__(self, burn_in, memory_size=40, SEED=678, GAMMA= 0.99, K= 20 ):
...
def comp(sample_path, nS, nA, Qp, QG, QL, EmaxQ_array, mean_reward_array,
         envstates, envactions, envf, envk, envl):
    for t in range(len(sample_path) - 1, -1, -1):
        w = sample_path[t] 
        for s_idx in  range(nS):
             for a_idx in range(nA):
                action = envactions[s_idx, a_idx]
                # j= action[0]- envi
                # k= action[1]- envj
                l= w[0]+envk
                m= w[1]+envl
                next_state =  envf[s_idx,a_idx,l, m]
...
class LBQL(Qlearning):
    ...
    def solve_QG_QL_DP(self, sample_path):
        ...
        self.QG[tau, :, :] = np.copy(Qp)
        self.QL = np.zeros((tau + 1, self.env.nS, self.env.nA))
        self.QL[tau, :, :] = np.copy(Qp)
        self.QD = np.zeros((tau + 1, self.env.nS, self.env.nA))
        self.backward_induction(Qp,sample_path)
    
    
    def Average_EmaxQa(self, Qp):
        ''' Computes an estimated expected value using simulation '''    
        dim_s = self.sample.shape[0]
        p = 1/dim_s*np.ones(dim_s)
        EmaxQa=np.dot(np.max(Qp[self.env.f[:,:,self.sample[:,0]+self.env.k,self.sample[:,1]+self.env.l], :],axis=3).reshape((self.env.nS,
             self.env.nA,dim_s)), p)
    ...
    def backward_induction(self,Qp, sample_path): 
        '''
        Solves the deterministic perfect information relaxation problem via backward induction 
        '''  
        EmaxQ_array, mean_reward_array = self.Average_EmaxQa(Qp)
        self.QG, self.QL = solve_inner_DP(sample_path, self.env, Qp, EmaxQ_array, mean_reward_array)
    ...
    def train(self, interactive_plot = False, verbose = False):
        ...
        for step in range(self.num_steps):
            ...
            q_values =  self.Q[state, :] #+ self.U[state, :] - self.L[state,:]           
            action, action_idx = self.epsilon_greedy_policy(q_values)
            self.most_visited_sa.append((state,action_idx))
            self.count[state, action_idx] += 1
            ...
            self.Q_learning[state, action_idx] +=  self.lr *(reward + self.gamma* np.max(self.Q_learning[newState, :]) -\
                                                   self.Q_learning[state, action_idx])
...
if __name__=='__main__':
    ...
    # To draw the bounds set interactive_plot= True 
    
    from collections import Counter
    Output = Counter(agent.most_visited_sa)
    print(Output.most_common(50))
    ...
    lbl=[0,0,20,40,60,80,100]#np.arange(0,20000/1000+1,5)
    plt.plot(agent.Lsa_2[0:40001], label='LBQL-L', lw=2)
    plt.plot(agent.Qsa_2[0:40001], label='LBQL-Q\'', lw=2)
    plt.plot(agent.Usa_2[0:40001], label='LBQL-U',lw=2)
    plt.plot(agent.QLsa_2[0:40001], label='QL-Q',lw=2)
    # plt.legend(loc='best')
    ...
    sns.set(font_scale=1.75)
    fig, ax = plt.subplots(1,1)
    length= len(agent.Lsa_1)
    qstar_sa_1 = [ Qstar[agent.sa1]]*length
    #lbl=np.arange(0,length/1000,1).astype(int)
    lbl=[0,0,10,20,30,40]#np.arange(0,20000/1000+1,5)
    plt.plot(agent.Lsa_1[0:40001], label='LBQL-L', lw=2,  alpha=0.8)
    plt.plot((np.array(agent.Qsa_1[0:40001])), label='LBQL-Q\'', lw=5,alpha=0.8)
    plt.plot(agent.Usa_1[0:40001], label='LBQL-U',lw=2,  alpha=0.8)
    plt.plot(agent.QLsa_1[0:40001],color='k', label='QL-Q',lw=2, linestyle='-',alpha=0.8)
    plt.plot(qstar_sa_1[0:40001], label='Q*', color='k', linestyle=':',lw=2)
