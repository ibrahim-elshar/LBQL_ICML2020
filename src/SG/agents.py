...
class Qlearning():
    ...
    def __init__(self, ENV,
        ...
        self.Q =  np.zeros((self.env.nS, self.env.nA))# Qstar.copy() #self.prng.uniform(L,U)
    ...
    for t in range(len(sample_path) - 1, -1, -1):
        ...
        for state in range(nS):
            for action in range(nA):
                next_state = envf[state, action, w[0] + envrange_random_wind, w[1]]
...
class LBQL(Qlearning):
    ...
    def Average_EmaxQa(self):
        ...
        EmaxQa=np.dot(np.max(self.Q[ self.env.f[:,:,self.sample[:,0]+self.env.range_random_wind, self.sample[:,1]], :],axis=3),p)
        mean_reward=np.dot(self.env.r[ :, :,self.sample[:,0]+self.env.range_random_wind,self.sample[:,1], self.sample[:,2]],p)
        return self.gamma*EmaxQa, mean_reward
    
    
    def Averag_Ereward(self, state, action):
        ''' Computes an estimated expected value using simulation '''
        mean_reward = np.mean(self.env.r[state, action, self.sample[:,0]+self.env.range_random_wind, self.sample[:,1], self.sample[:,2]].ravel())
        return  mean_reward 
...
if __name__=='__main__':
    ...
    env.reset()
    agent = LBQL(env,1)
    q, ql,ep,nstps, et  =agent.train(interactive_plot= True)