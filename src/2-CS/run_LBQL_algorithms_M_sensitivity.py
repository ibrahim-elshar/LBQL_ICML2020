...
Q_list = {}
QL_list= {}
Labels = ['M=1', 'M=10', 'M=50', 'M=150','M=200']
M_list = [1,10,50,150,200]
agents =[LBQL,
         LBQL,
         LBQL,
         LBQL,
...
for i in range(n_agents):
    for j in range(times):
        ...
        agent = agents[i](env,SEED=SEED_, M=M_list[i])#, SEED_ENV=SEED_ENV_)
...
Alg = np.repeat(['M=1', 'M=10', 'M=50', 'M=150','M=200'],array.shape[1]*array.shape[2])
...
sns_plot=sns.lineplot(x="Epoch", y="Reward", hue='Algo',data=df,ci=95, palette=['r','b', 'c', 'g','m'],
                      hue_order=['M=1', 'M=10', 'M=50', 'M=150','M=200'], style="Algo",
                      markers=False, dashes=True, lw=2)
#sns_plot.legend().set_visible(False)
plt.legend(title='', loc='lower right', labels= ['m =1', 'm =10', 'm =50', 'm =150','m =200'])
#sns_plot.legend().set_visible(False)
...
sns_plot=sns.lineplot(x="Epoch", y="RE", hue='Algo',data=df,ci=95,palette=['r','b', 'c', 'g','m'], 
                      hue_order=['M=1', 'M=10', 'M=50', 'M=150','M=200'],style="Algo",
                      markers=False, dashes=True, lw=2)
plt.legend(title='', loc='upper right', labels= ['m =1', 'm =10', 'm =50', 'm =150','m =200'])
#sns_plot.legend().set_visible(False)
plt.xlabel('Number of steps x'+str(at) )
plt.ylabel('Relative Error')
fig2 = sns_plot.get_figure()
fig1.savefig('results/'+str(run)+'/all RE.png')
df.to_pickle('results/' + str(run) + '/carsharing_df_M_Sensitivity.pkl')


mean_elapsed_time = np.mean(elapsed_time,1)
print('Mean run time for M=1',mean_elapsed_time[0])
print('Mean run time for M=10',mean_elapsed_time[1])
print('Mean run time for M=50',mean_elapsed_time[2])
print('Mean run time for M=150',mean_elapsed_time[3])
print('Mean run time for M=200',mean_elapsed_time[4])